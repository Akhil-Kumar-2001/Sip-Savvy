...
const cancelOrder = async (req, res) => {
  try {
    ...
    for (let product of orderDetails.products) {
      if (
        product.product_id &&
        typeof product.product_id.productQuantity === "number" &&
        ...
        console.error(
          `Invalid product quantities for product ID ${product.product_id}: productQuantity = ${product.product_id.productQuantity}, product_quantity = ${product.product_quantity}`
        );
      }
    }
...
const Invoice = async (req, res) => {
  try {
    ...
    doc
      ...
      .fontSize(8)
      .text(
        "Where every bottle tells a tale, and every sip is a celebration.",
        {
          align: "center",
          margin: 10,
        }
    ...
    doc
      .fontSize(10)
      .fillColor("blue")
      .text(`Invoice #${orderDetails.order_id}`);
    doc.moveDown();
    doc.moveDown();
    ...
    doc.fontSize(10).fillColor("black").text(`Address: Perumon, Kollam`);
    ...
    const tableData = {
      ...
      rows: orderDetails.products.map((product) => {
        ...
        const total = Math.round(
          price * (1 - discount / 100) * quantity - coupondiscount.toFixed(2)
        );
